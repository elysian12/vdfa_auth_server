...
app.get('/api/v1/Authentication/GetOTP/:phoneNumber', async (req, res) => {
    ...
    var res = await client.verify.v2.services(process.env.TWILIO_SERVICE_ID)
        .verifications
        .create({ to: `+91${phoneNumber}`, channel: 'sms', appHash: '+bmLh8lZj08' }).then(
            (verification) => { res.status(200).json({ msg: "Working", phoneNumber: verification.to, channel: verification.channel }) }
        );
});
...
app.post('/api/v1/Authentication/VerifyOTP', async (req, res) => {
    ...
    await client.verify.v2.services(process.env.TWILIO_SERVICE_ID)
        .verificationChecks
        .create({ to: `+91${to}`, code: otp }).then((data) => {
